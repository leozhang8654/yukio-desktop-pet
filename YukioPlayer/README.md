...
原来每套动作的 4 帧是分别生成的画，整幅线条（连桌腿、椅子）都有 1 像素级漂移，轮播就会“呼吸抽搐”，所以只能长时间停在一帧上。现在每个状态只用其中一张已确认的底图，由 `tools/motion/make_motion.py` 做局部平滑变形生成连续动作：桌椅和身体逐像素不动（生成时自动检查桌腿一带零变化），手、笔、头、眼在 2 像素以内移动，每帧 50–70 ms。

| 状态 | 动作 |
| --- | --- |
| `thinking` | 托腮，头绕着托下巴的手慢慢歪一点又回来，偶尔往上看，慢眨眼（幅度最小） |
| `read_file` | 视线和指头沿一行从左往右，读完回到行首，头跟着微微转 |
| `view_image` | 拿放大镜的手沿小椭圆慢慢移动，视线跟着镜片 |
| `write_file` | 握笔的手一边小幅上下抖一边往右写，写完一行回到左边 |
| `verify` | 在两份文件之间左右转头（五官比轮廓多移一点，像真的转头），手按着正在看的那份 |
| `read_web` | 手指在平板上往上划两下，停一会儿，眼睛跟着往下扫 |
| `default_work` | 两只手不规则地轻敲键盘、停一小会儿再敲，眼睛在屏幕上扫 |
| `respond` | 递出报告一次后停住，只眨眼，不来回递纸 |
| `idle` | 偶尔向左、向右看一看，头跟着微微歪，眨眼 |
| `failed` | 垂眼一次后停住，慢慢叹气（头往下沉半个像素）、慢眨眼 |

所有状态都会眨眼：眼皮用各自姿势脸颊的肤色，闭眼线用各自睫毛的颜色。要改动作或幅度，编辑 `tools/motion/make_motion.py` 里对应的函数后重新运行（需要 numpy、opencv-python、Pillow），图条写到 `Resources/Assets/motion/`；`--eyes`、`--sheet`、`--html` 分别画眨眼检查图、局部放大对照图和新旧动作并排的预览页。删掉 `Resources/Assets/motion/motion.json` 就回到原来的图条。
...
- 主循环 30 Hz（小幅动作一帧约 67 ms），本机实测 CPU 约 1.5%；约 300 帧预先解码成独立小位图，内存约 90 MB。
