...
一只雪绪，根据 **Claude Code** 当前在做的事自动切换动作：七类活动各有动作，其余工作显示电脑桌，出错时沮丧，要你拿主意时立起问号卡，答完举起勾选卡——牌子会一直举着等你，点她一下就跳回那条聊天；没有任务时空闲。
...
| 下行（没有清单时） | 当前活动 | 正在进行的工具调用：`编辑 main.swift`、`$ swift test`、`浏览 developer.apple.com`、`等你回答 · 点我打开对话`… |
...
- **点卡片正文**：打开那条聊天，同时收起这张卡（和点举着的牌子是同一套跳转）。
- **点 ✕**：只收起这一张。收起是按“这一轮”记的：那条聊天下一轮有动静时会重新出现。
- **右键一张卡**：“不再提醒这条聊天”——这次运行内它不再出卡。
- 最多显示 3 张，其余折进最上面的“还有 N 条”，点一下展开（最多 6 张），再点“收起”。
- 菜单里的“头顶显示别的聊天”可以整摞关掉；暂停跟随、播放模拟演示时也不显示。气泡关掉时这摞卡从头顶线开始叠。
- 卡片之间的缝隙照样点得穿透过去，只有卡片本身收点击。

排序和她自己跟谁用的是同一套档位，所以**紧挨着气泡那张就是“她下一个会去显示的”**。这套做法参考了 ChatGPT 桌面版那只宠物的通知盘（宠物旁边一叠卡、单卡可开可消可静音），区别是我们不另起一套样子：气泡本身就是那摞卡的第一张，别的聊天接着往上叠。
