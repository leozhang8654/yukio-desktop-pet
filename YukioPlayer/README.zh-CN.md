...
一只雪绪，根据你的编码助手——**Claude Code**、**DeepSeek 的 Deep Code CLI** 或 **GPT 的 Codex**，在菜单「跟随的助手」里挑——当前在做的事自动切换动作：七类活动各有动作，其余工作显示电脑桌，出错时沮丧，要你拿主意时立起问号卡——问题就抄在她身边那张卡上，可以当场回答——答完举起勾选卡——牌子会一直举着等你，点她一下就跳回那条聊天；那条聊天要是已经开在你眼前，就不举牌了；没有任务时空闲。
同时开着好几个聊天时，谁答完、谁在等你拿主意，就先显示谁；其余有话要说的，按同一张卡的样子叠在气泡上面，点一张就去那条聊天。也可以在菜单里挑定一条，让她只跟那条。每个动作都在小幅、连续地动（写字、敲键盘、转眼、眨眼），头顶的小气泡显示大任务、当前任务和进度。
原生 Swift / AppKit，只需 Xcode Command Line Tools，无第三方依赖。

## 个人助手（0.3.1）

主窗口提供首页、提醒、个性化和规划中的扩展页。单次提醒支持编辑、删除、稍后 5 分钟和确认完成；本地保存，关闭窗口后继续计时，唤醒或重新启动后补提醒。`--pet-only` 启动时隐藏主窗口，`--assistant-only` 启动时隐藏桌宠。参见[助手说明](../docs/ASSISTANT.md)。

## 构建与运行

```sh
cd YukioPlayer
swift test                     # 163 个测试（路由、眨眼时序、分类、解析、文件跟随、资源、气泡文字、动作时间线、聊天对应与选择、抄题与回答）
./scripts/build-app.sh         # 生成 build/Yukio.app（资源与图标已打包进应用）
open build/Yukio.app
```

运行后：屏幕右下角出现雪绪；菜单栏出现她的小头像。菜单里有当前状态、正在跟的聊天、模拟演示、暂停跟随、跟随的助手（Claude Code／DeepSeek／GPT，或自动）、跟随的聊天（挑一条）、头顶显示任务、头顶显示别的聊天、在这儿回答问题、大小、回到右下角、退出；举着勾选卡时最上面多两条“打开这条聊天并放下牌子”“先放下牌子，不打开聊天”，立着问号卡时多一条“打开这条聊天去回答”。大小是一条滑条（50%–200%，每 5% 一档）：拖着走，雪绪当场跟着变大变小，脚下位置不动。菜单里另有「语言」子菜单（English／中文），默认英文，选择会记住；已经写进事件里的说明文字（如“阅读 main.swift”）要到下一条事件才换语言。

设置与菜单入口：

1. 菜单栏小头像。首次运行默认放在靠右处（距右边缘约 260 点）；按住 ⌘ 拖动可换位置，系统会记住。
2. 在雪绪身上右键（或 control-点击），打开助手；侧边栏可进入桌宠设置。
...
| `--replay 会话.jsonl` | 用虚拟时钟回放一份会话记录，打印雪绪会显示的状态序列；格式（Claude／Deep Code／Codex）按文件内容自动认；加 `--with-bubble` 同时打印气泡文字（含标题与文件名） |
| `--watch 秒数` | 实时跟随会话记录，打印事件、写入延迟和状态切换（不打印对话内容） |
| `--source auto\|claude\|deepseek\|gpt` | `--watch` 与 `--chats` 跟哪一家；不写就用菜单里那个设置 |
| `--menu` | 不开窗口，打印菜单（含子菜单）的文字；可加 `-source gpt`、`-language zh` 看别的设置下长什么样 |
...
| `question_for_user` | C 立起问号卡 | `AskUserQuestion`：任务仍在进行，但在等你拿主意；问题抄在她身边那张卡上，可以当场答（见「在她这边回答」），答完卡片自己收 |
...
每个状态复用 [`sources/approved-animation-rig`](../sources/approved-animation-rig/README.md) 内已验收的 384×416 主图和最终 SAM 小部件蒙版。头颈固定，不额外点头。读书时视线沿文字移动，写字时手带着笔动，看图时手带着放大镜动；保留原来的袖口、家具和前景遮挡。

`tools/motion/make_motion.py` 现在调用 `approved_motion.py`，保留原 50 fps 时间轴、开场/循环位置和 v4 六级独立眼皮。使用 `--out build/neck-stability-sam/after` 输出候选，再用 `check_motion.py` 的 `--before`、`--after`、`--out` 和可选 `--gif` 核验全部状态并生成对比预览，核验后再替换正式资源。无需再超分。历史算法必须显式加 `--legacy` 才会运行，不用于正式制作。
...
- **立着问号卡时点她**：打开那条聊天去回答。和勾选卡不同，卡片不收——问题还等着你答，等你答完它自己收。菜单里对应“打开这条聊天去回答”。不想跑过去的话，问题就抄在她身边，直接在那儿答（见下一节）。
- 其余状态点她不做事；拖动、右键打开助手、点击穿透都和原来一样（挪动超过 3 点算拖动，不算点击）。
...
立着问号卡时，那道题就抄在她身旁：小标题、折成最多六行的问题正文、一行一个的选项。
太长的问题在末尾截断（完整的那份在聊天里），选项最多列六个，多出来的写成「还有 N 个选项在聊天里」。
卡片立在她左边还是右边看哪边放得下，不会压住她，也不会压住头顶那摞气泡。

- **点一个选项**，那个选项的文字就是答案。按数字键 1–9 是同一件事。
- **多选题**点过的选项打上勾，按 ⏎ 一起送出，中间用「、」连起来（英文界面是 ", "）。
- **自己写一句**：下面那个输入框是真正的系统文本框，输入法照常用，写完按 ⏎（或点 ⏎ 那个键）。
- **✕**：这一道题先不在这儿答，问号卡照举；换一道题卡片会重新立起来。
- **「打开聊天 ›」**：跳过去自己答，和点她一样。

答案是怎么送过去的：先复制一份到粘贴板，用深链打开那条聊天（Claude／Codex 会被带到最前面），
**确认最前面的确实是它**，再替你按 ⌘V 和回车。这需要系统的**辅助功能权限**（第一次会弹系统面板，
在「系统设置 › 隐私与安全性 › 辅助功能」里勾上）。没有权限、那个应用没被带到前面、或者这中间你又切走了，
她一个键都不按：答案就留在粘贴板里，聊天也已经打开，卡片上写着「已复制 · 到聊天里 ⌘V」。
绝不会往别的应用里乱按键。粘贴板随后会放回原样——除非这期间你自己复制了别的东西。

跳不过去的聊天（Deep Code 跑在终端里）只复制。模拟演示里不会真的送出去。

设置里的**「在这儿回答问题」**可以整张卡关掉，关掉后问号卡还是老样子：点她跳过去答。
`./.build/release/YukioPlayer --question out.png` 离屏画出这张卡（单选、多选、没有选项、已答过四种），
并打印点击分区——本机没有屏幕录制权限，界面就是这样自查的。
...
菜单里的 **跟随的助手** 挑一家，选择存在 `UserDefaults` 的 `source` 键里（与 Windows 版同一个键名）：

| 菜单 | 读哪里 | 备注 |
| --- | --- | --- |
| 自动（谁在干活跟谁）——默认 | 下面三家都跟 | 三家的聊天按同一套规则竞争（见「多个聊天同时跑」） |
| Claude Code | `~/.claude/projects/<项目>/<会话>.jsonl`（尊重 `CLAUDE_CONFIG_DIR`），外加可选的 hooks 收件箱 | 有会话标题、任务清单、❓`AskUserQuestion` |
| DeepSeek（Deep Code） | `~/.deepcode/projects/<项目码>/<会话>.jsonl` 与同目录的 `sessions-index.json`（尊重 `DEEPCODE_CONFIG_DIR`） | 「等你批准」只写在索引里 |
| GPT（Codex） | `~/.codex/sessions/<年>/<月>/<日>/rollout-*.jsonl`（尊重 `CODEX_HOME`） | 一条聊天一个文件；桌面版与命令行写的是同一份 |

换一家时先整个清空（挑定的聊天、举着的牌子都属于上一家），再从新的一家重新回放。

### 默认：会话记录（只读，零配置）

只读，不修改这几个工具的任何文件或设置。启动时回放最近 15 分钟内活跃会话的末尾，恢复“此刻在做什么”。

注意：这些是各个工具在本地写的会话记录，**不是公开 API**，格式可能随版本变化；解析失败时只会少事件，不会崩溃，并按失联规则回空闲。子代理（sidechain）的活动不驱动主角色。

Codex 在同一个文件里写两套记录，两套都用：`response_item`（送给模型的那一份）在工具**开始**时就写下，动作跟得上；`event_msg`（`item_completed`、`task_complete`、`turn_aborted`）是事后的回执，用来补成败和「一轮结束」。桌面版几乎所有事都走一个 `exec` 工具，参数是一段 JavaScript，所以真正在干什么要从那段正文里读：`tools.exec_command({cmd:…})` 走与 Claude `Bash` 同一套命令分类，`tools.apply_patch` 是改文件，`tools.view_image` 是看图，`tools.write_stdin` 延续上一个动作。
...
- 点击跳转认桌面版 Claude 的会话，也认 Codex 的聊天（`codex://threads/<会话 ID>`，ID 就是 rollout 文件名里的那个，Codex 应用自己的日志里叫 `threadId`）。在终端里跑的助手没有对应的聊天窗口，点击只会把那个应用带到前面；Deep Code 没有窗口可跳，那条菜单就不显示，点击只放下牌子。Codex 这条链接还没从头点通过一次，需要目测。
- Codex 的会话记录里没有聊天标题，所以聊天列表和气泡里用请求的第一行当名字（连第一行都不在回放范围内时用会话 ID）。
...
- 主循环以 30 Hz 读取 50 fps 素材时间轴。发布包按需加载无损分页，页缓存上限 32 MiB；进程总内存还包括帧、窗口和其他资源。
