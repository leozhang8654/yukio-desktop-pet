...
到 [Releases](https://github.com/leozhang8654/yukio-desktop-pet/releases/latest) 下载 `Yukio-0.2.1-Windows.exe`，放哪儿都行，双击就开。Python、Pillow、素材都打包在里面了，需要 Windows 10 或更新的 64 位系统。
...
| 在这儿回答问题 | 立着问号卡时，问题连同选项抄在她身边：点一个选项（或自己写一句按回车），雪绪替你把答案送进那条聊天（见「在她这边回答」）。菜单 › 在这儿回答问题 可以关掉 |
...
| `question_for_user` | C 立起问号卡 | `AskUserQuestion`（问题和选项抄到她身边那张卡上）；索引状态 `ask_permission`（等你批准）、`waiting_for_user`——这两种没有题目文字，不立卡 | AskUserQuestion |
...
立着问号卡、而且来源给出了题目（Claude Code 的 `AskUserQuestion`、Codex 的 `request_user_input`）时，
那道题就抄在她身旁：小标题、折成最多六行的问题正文、一行一个的选项。太长的问题在末尾截断（完整的那份在聊天里），
选项最多列六个，多出来的写成「还有 N 个选项在聊天里」。卡片立在她左边还是右边看哪边放得下，始终留在工作区里。

- **点一个选项**，那个选项的文字就是答案。
- **多选题**点过的打上勾，按回车一起送出。
- **自己写一句**：下面那个输入框是真正的 Windows EDIT 控件（单独一扇小窗口），输入法照常用；回车送出，Esc 先清空、再按一次收卡。
- **✕**：这一道题先不在这儿答；换一道题卡片会重新立起来。
- **「打开聊天 >」**：跳过去自己答。

答案是怎么送过去的：先复制一份到粘贴板，用桌面版 Claude 注册的深链打开那条聊天，
**等到最前面的确实是 `Claude.exe`**，再用 `SendInput` 的 Unicode 模式把答案逐字打进去（不看键盘布局、也不经过输入法），
最后按一下回车。Windows 上这件事不需要额外授权。那个应用 2.5 秒内没到最前面、或者这中间你又切走了，
**一个键都不会按**：答案就留在粘贴板里，卡片上写着「已复制 · 到聊天里 Ctrl+V」。绝不会往别的应用里乱按键。
跳不过去的聊天（Deep Code 跑在终端里，命令行版 Claude Code 也一样）只复制。模拟演示里不会真的送出去。

菜单 › **在这儿回答问题** 可以整张卡关掉，关掉后问号卡还是老样子。
`python -m yukio --question out.png` 离屏画出这张卡（单选、多选、没有选项、已答过四种）并打印点击分区——
本机没有 Windows，界面就是这样自查的。
